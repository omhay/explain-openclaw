...
5. Is actively integrated into web search (`src/agents/tools/web-search.ts:1083,1110,1138,1164`), web fetch (`src/agents/tools/web-fetch.ts:250-271`), cron hooks (`src/cron/isolated-agent/run.ts:311-346`), Discord (`src/discord/monitor/message-handler.process.ts:181-190`), and Slack (`src/slack/monitor/room-context.ts:15-19`)
...
| Web search wrapping | `src/agents/tools/web-search.ts:1083,1110,1138,1164` | All search snippets wrapped via `wrapWebContent()` |
...
| Discord metadata isolation | `src/discord/monitor/message-handler.process.ts:181-190` | Channel topics wrapped via `buildUntrustedChannelMetadata()` |
